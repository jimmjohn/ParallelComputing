...
{
  int istep;
  int nstep = 1e4; // number of time steps
...
    cv::Mat image(nj, ni, CV_8UC3);

    // Normalize temperature values to 0-255 range
    float max_temp = 0.0f;
    for (int i = 0; i < ni * nj; i++) {
      max_temp = std::max(max_temp, temp1[i]);
    }

    for (int i = 0; i < ni; i++) {
      for (int j = 0; j < nj; j++) {
          int pixel_value = static_cast<int>(temp1[I2D(ni, i, j)] / max_temp * 255);
          image.at<cv::Vec3b>(j, i) = cv::Vec3b(pixel_value, pixel_value, pixel_value);
      }
    }
